"""
CP-MVP2-05 — Claude automation-step provider adapter tests.

Mirrors tests/test_cp_mvp2_03_claude_provider.py and
tests/test_cp_mvp2_04_claude_provider.py exactly: every test mocks
`llm.automation_client.subprocess.run` and
`llm.automation_client.shutil.which` — no real `claude` process, no
...
from llm.automation_client import ClaudeAutomationClient
...
SAMPLE_CONTEXT = {
    "testcase_id": "TC-REQ-ACO-03-01",
    "requirement_id": "REQ-ACO-03",
    "test_steps": ["Enter first name"],
    "expected_result": "form accepted",
...
def _client(monkeypatch):
    ...
    with patch("llm.automation_client.shutil.which", return_value="/usr/bin/claude"):
        return ClaudeAutomationClient()


def test_missing_executable_raises_provider_error(monkeypatch):
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    with patch("llm.automation_client.shutil.which", return_value=None):
...
def test_anthropic_api_key_present_raises_before_touching_cli(monkeypatch):
    ...
    with patch("llm.automation_client.shutil.which") as which_mock:
...
def test_successful_invocation_parses_candidates(monkeypatch):
    ...
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, envelope, "")) as run_mock:
        candidates = client.propose_steps(SAMPLE_CONTEXT)
...
def test_argv_is_a_list_not_shell_and_uses_restricted_mode(monkeypatch):
    ...
    envelope = _envelope(json.dumps([VALID_CANDIDATE]))
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, envelope, "")) as run_mock:
        client.propose_steps(SAMPLE_CONTEXT)
...
def test_non_zero_exit_code_raises_provider_error(monkeypatch):
    client = _client(monkeypatch)
    with patch("llm.automation_client.subprocess.run", return_value=_completed(1, "", "cli error")):
...
def test_timeout_raises_provider_error(monkeypatch):
    client = _client(monkeypatch)
    with patch("llm.automation_client.subprocess.run", side_effect=real_subprocess.TimeoutExpired(cmd="claude", timeout=120)):
...
def test_malformed_envelope_json_raises_provider_error(monkeypatch):
    client = _client(monkeypatch)
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, "{not json", "")):
...
def test_result_not_a_list_raises_provider_error(monkeypatch):
    ...
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, envelope, "")):
...
def test_dataset_context_with_shell_metacharacters_is_passed_as_single_safe_argument(monkeypatch):
    ...
    dangerous_context["test_steps"] = ["; rm -rf / #$(whoami)`id`\nNEWLINE 'quote' \"dquote\" ☺"]
    envelope = _envelope(json.dumps([VALID_CANDIDATE]))
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, envelope, "")) as run_mock:
        client.propose_steps(dangerous_context)
    call_args = run_mock.call_args.args[0]
    assert run_mock.call_args.kwargs.get("shell") is False
    prompt_arg = call_args[call_args.index("-p") + 1]
    decoded = json.loads(prompt_arg)
    assert decoded["test_steps"] == dangerous_context["test_steps"]
...
def test_stderr_present_alongside_success_is_ignored_not_merged(monkeypatch):
    client = _client(monkeypatch)
    envelope = _envelope(json.dumps([VALID_CANDIDATE]))
    with patch("llm.automation_client.subprocess.run", return_value=_completed(0, envelope, "harmless diagnostic noise")):
        candidates = client.propose_steps(SAMPLE_CONTEXT)
    assert candidates == [VALID_CANDIDATE]
