"""
Post-MVP2 Enhancement 01 -- test-data access layer (backlog item E-16 /
governing instruction sec. 16).

Automation never hard-codes a business dataset. It loads a dataset strictly
by `Dataset ID` from the frozen, persisted, authoritative-for-automation
`testdata/generated/` JSON corpus (via the unmodified `testdata.persist`
module) -- never from the human-consumable `.xlsx` workbook (which is a
generated view, not a source; see `testmgmt/generate_workbooks.py`'s own
module docstring for the full provenance rule). This is what prevents the
Excel/JSON "two uncontrolled sources of truth" problem the governing
instruction (sec. 12) warns against.
"""
