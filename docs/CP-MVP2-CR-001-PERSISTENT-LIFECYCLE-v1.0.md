...
* **Status:** **PROPOSED — PENDING HUMAN + DI REVIEW. NOT APPROVED. NOT IMPLEMENTED.**
...
3. A newly-defined (see §4 below — **not yet approved**) RBTP artifact and its persistence.
...
## 4. RBTP — Terminology Investigation (must be resolved before implementation)

A repository-wide search (code, every Markdown document under `docs/`, `README.md`, `ClaudeInstructions/`, and the two `ChatGPT_MVP3_RelatedChat...` reference documents at the repository root) found **zero** existing occurrences of "RBTP" anywhere in this project. It is not a term this project has previously defined, used, or approved.

Per this CR's own instruction ("Do NOT invent a conflicting definition... If RBTP is not currently defined, explicitly document the definition proposed for approval"), the following is a **proposed definition only — not yet approved, not to be implemented until explicitly authorized**:

> **RBTP (proposed): Requirement-Based Test Plan.** A persisted, per-requirement (or per-requirement-cluster) planning/governance artifact, sitting conceptually between the Requirement and the Testcase layer, that records: the testing *intent* for that requirement (why particular scenario types are/are not justified), a priority/risk classification, a summary of which scenario types (positive/alternate/exceptional) are covered vs. still needed, known dependencies (shared preconditions, data, or setup another requirement's testing already establishes), identified coverage gaps, and execution relevance (whether this requirement's testing is currently eligible to reach real browser execution, per the CP-06 eligibility model). It does not restate requirement text (that remains the Approved SRS's role) and does not restate testcase content (that remains the Testcase artifact's role) — it is the record of *test planning reasoning*, one layer of deterministic governance above raw testcase generation.

This definition is deliberately narrow and additive: it does not conflict with, replace, or duplicate any existing frozen schema (`Testcase`, `GovernedTestcase`, `TestDataSet`, `PlaywrightArtifact`). **This CR asks the human reviewer to explicitly approve, reject, or amend this definition before any RBTP implementation work begins.**
...
REQUIREMENT ↔ TESTCASE TRACEABILITY ARTIFACT (persisted)
     ↓
RBTP ARTIFACT (persisted — pending definition approval, §4)
     ↓
...
| §7 RBTP | Persist the RBTP artifact — **pending definition approval, §4.** | RBTP Artifact |
...
* **New schemas required** (none yet implemented): Traceability Artifact schema (REQ↔TC, TC↔Data, Automation chain), RBTP schema (pending §4 approval), Dependency Matrix schema, Execution Summary schema.
...
1. **RBTP definition (§4):** approve, reject, or amend the proposed definition before any RBTP implementation begins.
...
1. The RBTP definition (§4) is explicitly approved, rejected, or amended by the human reviewer.
